...
use handlers::handle_file;
use pretty_env_logger;
use std::error::Error;
use util::cli::{get_static_path, ARGS, DB_POOLS};
...
fn app_config(config: &mut web::ServiceConfig) {
    config.service(
        web::scope("")
            ...
            .service(web::resource("/files/{file_name}").route(web::get().to(handle_file)))
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    indexing(false);
    let _ = &*DB_POOLS; // 确保 LazyLock 初始化
